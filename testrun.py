...
class SetGetTest(ServiceTester):
    script = "app/scripts/determineuseraction.py"
    def run(self):
        time.sleep(1)
        
        DetermineUserAction = self.service("DetermineUserAction")
        DetermineUserAction.set(0)
        assert DetermineUserAction.get() == 0
        DetermineUserAction.set(2)
        assert DetermineUserAction.get() == 2
        return True
...
class StopTest(ServiceTester):
    ...
    def run(self):
        ...
        goterror = False
        try:
            DetermineUserAction.stop()
            time.sleep(1)
            DetermineUserAction.set(1)
        except RuntimeError:
